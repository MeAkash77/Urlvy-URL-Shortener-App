...
export default function Links() {
  ...
  useEffect(() => {
    api
      ...
      .then((res) => {
        ...
        // Ensure all links have a valid clicks array
        const safeLinks: LinkItem[] = data.map((l: any) => ({
          ...l,
          clicks: Array.isArray(l.clicks) ? l.clicks : [],
        }));
        setLinks(safeLinks);
      })
      .catch(() => {
  ...
  const add = async () => {
    ...
    try {
      ...
      const newLink = {
        ...(data.data ?? data),
        clicks: Array.isArray(data.data?.clicks) ? data.data.clicks : [],
      };
  ...
  const allClicks = useMemo<Click[]>(
    () =>
      (links ?? [])
        .flatMap((l) => l.clicks)
        .filter((c): c is Click => !!c?.createdAt && !!c.userAgent),
    [links]
  );
  ...
  // ✅ FIXED: Defensive code to prevent .length of undefined
  const topLinks = useMemo(() => {
    if (!Array.isArray(links)) return [];
    return [...links]
      .filter((l): l is LinkItem => !!l && Array.isArray(l.clicks))
      .sort((a, b) => (b.clicks?.length ?? 0) - (a.clicks?.length ?? 0))
      .slice(0, 5)
      .map((l) => ({
        name: l.slug,
        value: l.clicks?.length ?? 0,
      }));
  }, [links]);
  ...
  return (
    <>
      <Head>
        ...
        <meta name="description" content="Manage your shortened URLs and view their stats" />
      </Head>
          ...
          <div className="grid gap-8 md:grid-cols-2">
            ...
            <ChartCard title="Device Split">
              {chartsReady ? (
                <ResponsiveContainer width="100%" height={200}>
                  <PieChart>
                    <Pie data={deviceSplit} dataKey="value" nameKey="name" outerRadius="60%" label>
                      {deviceSplit.map((_, i) => (
            ...
            <ChartCard title="Top 5 Most-Clicked Links">
              {chartsReady ? (
                <ResponsiveContainer width="100%" height={200}>
                  <PieChart>
                    <Pie data={topLinks} dataKey="value" nameKey="name" outerRadius="60%" label>
                      {topLinks.map((_, i) => (
...
function Grid({ links }: { links: LinkItem[] | null }) {
  ...
  if (links.length === 0) {
    return <p className="text-center text-muted-foreground">No links to show.</p>;
  }
  return (
    <div className="grid gap-6 sm:grid-cols-2">
      {links.map((link) => (
